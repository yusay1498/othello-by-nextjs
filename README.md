...
このプロジェクトでは、Claude CodeやGitHub CopilotなどのAIコーディングアシスタント向けのカスタムインストラクションを提供しています：

- **ファイル:** `.github/copilot-instructions.md`
- **目的:** プロジェクト固有のコーディング規約、コミットメッセージ形式、PR作成ガイドラインを定義
- **重要な制約:**
  - PRタイトルは英語で記述
  - PRディスクリプション、コミットメッセージ、コード内コメントは日本語で記述
  - 1つのPRには1つの機能のみ
  - コミットメッセージは `type(scope): メッセージ` 形式（日本語）

これらの制約は、カスタムインストラクションをサポートするAIコーディングアシスタントによって自動的に読み込まれます。
